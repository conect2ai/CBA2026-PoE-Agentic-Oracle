...
def fraud_investigator_node(state: PoEState) -> PoEState:
    ...
    try:
        llm = ChatOllama(model="llama3.1:8b", temperature=0, base_url="http://localhost:11434") # Temperatura baixa para ser analítico
        prompt = f"""
...
def judge_oracle_node(state: PoEState) -> PoEState:
    ...
    try:
        llm = ChatOllama(model="llama3.1:8b", temperature=0.1, base_url="http://localhost:11434")
        prompt = f"""
